...
TRAIN_LABEL_SAVE_COARSE = FILEPATH + 'coarse/train_images_coarse'
TEST_INPUT_SAVE = FILEPATH + 'coarse/test_images'
TEST_LABEL_SAVE_COARSE = FILEPATH + 'coarse/test_images_coarse'
...
def return_datatset_train():
    ...
    labels = tf.data.FixedLengthRecordDataset(
      TRAIN_LABEL_SAVE_COARSE, COURSE_SHAPE[0]* COURSE_SHAPE[1]).map(decode_label_coarse)
    return tf.data.Dataset.zip((images, labels))

def return_datatset_test():
    images = tf.data.FixedLengthRecordDataset(
      TEST_INPUT_SAVE, BASE_INPUT_SHAPE[0] * BASE_INPUT_SHAPE[1] * 3).map(decode_image)
    labels = tf.data.FixedLengthRecordDataset(
      TEST_LABEL_SAVE_COARSE, COURSE_SHAPE[0]* COURSE_SHAPE[1]).map(decode_label_coarse)
    return tf.data.Dataset.zip((images, labels))
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse):
        ...
        for i,v in enumerate(CONVOLUTIONS):
            ...
            curr_layer = BatchNormLayer(Conv2d(conv_pointers[-1],
                CONVOLUTIONS[i], (5, 5),strides = strides, name=
                'c_conv1_%s'%(i)),
